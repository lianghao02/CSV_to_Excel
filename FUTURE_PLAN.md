...
1. **`index.html`**: 網頁骨架。引入了 `SheetJS` (CDN) 用於處理 Excel。
2. **`style.css`**: 樣式表。使用 CSS 變數 (`:root`) 管理顏色，方便未來更換主題色。
3. **`script.js`**: 核心邏輯。
    - **`readCSV()`**: 使用 `FileReader` 讀取檔案，目前設定為 `utf-8` 編碼。若遇到舊式銀行檔案出現亂碼，可能需改為 `big5`。
    - **`cleanData()`**: 資料清理的核心。目前使用正則表達式 (`Regex`) 移除 `+` 和 `.00`。
    - **`XLSX.utils`**: 呼叫 SheetJS 的功能來產生 Excel。關鍵在於將儲存格格式設為文字 (`z: '@', t: 's'`) 以保留帳號前導零。
